#!/usr/bin/env python3
"""
Unit test to verify get_advanced_defense_stats_weekly migration structure.
...
def test_defense_weekly_stats_structure():
    """Test that get_advanced_defense_stats_weekly uses build_player_stats_query correctly."""
    print("Testing get_advanced_defense_stats_weekly migration structure...")
    print("=" * 60)
    ...
    try:
        with patch('tools.metrics.info.build_player_stats_query') as mock_build:
            ...
            assert kwargs["table_name"] == "vw_advanced_def_analytics_weekly", f"Wrong table: {kwargs.get('table_name')}"
            print("   ✓ Correct table_name: vw_advanced_def_analytics_weekly")
            ...
            assert kwargs["player_names"] == ["Fred Warner"], f"Wrong player_names: {kwargs.get('player_names')}"
            ...
            assert kwargs["metrics"] == ["def_tackles_combined", "def_sacks", "def_interceptions"], f"Wrong metrics: {kwargs.get('metrics')}"
            print("   ✓ Passes through metrics")

            assert kwargs["order_by_metric"] == "def_tackles_combined", f"Wrong order_by_metric: {kwargs.get('order_by_metric')}"
...
if __name__ == "__main__":
    success = test_defense_weekly_stats_structure()
