...
def test_offensive_stats_uses_build_player_stats_query():
    """Test that get_offensive_players_game_stats uses build_player_stats_query with correct params"""
    print("Testing get_offensive_players_game_stats uses build_player_stats_query...")

    # Mock build_player_stats_query
    with patch('tools.league.info.build_player_stats_query') as mock_build:
        mock_build.return_value = {"offGameStats": []}
        ...
        assert kwargs.get("table_name") == "nflreadr_nfl_player_stats", "Table name should be nflreadr_nfl_player_stats"
        ...
        assert kwargs.get("return_key") == "offGameStats", "Return key should be offGameStats"

        # Verify base columns
        base_columns = kwargs.get("base_columns", [])
        expected_base = ["season", "week", "player_display_name", "recent_team", "position"]
        ...
        assert kwargs.get("player_names") == ["Mahomes"], "player_names should be passed through"
        ...
        assert kwargs.get("metrics") == ["passing_yards"], "metrics should be passed through"
        assert kwargs.get("order_by_metric") == "passing_yards", "order_by_metric should be passed through"
        assert kwargs.get("limit") == 10, "limit should be passed through"
        assert kwargs.get("positions") == ["QB"], "positions should be passed through"

        print("✅ get_offensive_players_game_stats uses build_player_stats_query correctly")


def test_offensive_stats_return_value():
    ...
    with patch('tools.league.info.build_player_stats_query') as mock_build:
        ...
        mock_build.return_value = {"offGameStats": expected_data}

        mock_supabase = Mock()
        result = get_offensive_players_game_stats(supabase=mock_supabase)

        assert result == {"offGameStats": expected_data}, "Return value should match build_player_stats_query output"

        print("✅ Return value is correct")


def test_offensive_stats_signature_preserved():
    """Test that function signature is preserved"""
    print("Testing function signature preservation...")

    import inspect
    sig = inspect.signature(get_offensive_players_game_stats)
...
def test_offensive_stats_docstring_preserved():
    """Test that docstring is preserved"""
    print("Testing docstring preservation...")

    docstring = get_offensive_players_game_stats.__doc__
    assert docstring is not None, "Docstring should be present"
    assert "Fetch offensive weekly game stats for NFL players" in docstring, "Docstring should contain description"
...
if __name__ == "__main__":
    ...
    try:
        test_offensive_stats_uses_build_player_stats_query()
        test_offensive_stats_return_value()
        test_offensive_stats_signature_preserved()
        test_offensive_stats_docstring_preserved()
