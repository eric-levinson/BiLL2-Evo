#!/usr/bin/env python3
"""
Test script for get_advanced_receiving_stats_weekly migration.
...
def test_weekly_list_filtering():
    ...
    try:
        ...
        if "advReceivingStats" not in result:
            print("❌ FAILED: Result missing 'advReceivingStats' key")
            return False

        stats = result["advReceivingStats"]
        ...
        stats = result["advReceivingStats"]
        print(f"   ✓ Returned {len(stats)} records for Justin Jefferson")

        if stats:
            for stat in stats:
                week = stat.get("week", "N/A")
                player = stat.get("player_name", "N/A")
                yards = stat.get("receiving_yards", "N/A")
                print(f"     - Week {week}, {player}: {yards} yards")
