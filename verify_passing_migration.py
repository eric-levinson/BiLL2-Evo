#!/usr/bin/env python3
"""
Verify get_advanced_passing_stats migration by checking code structure.
...
from tools.metrics.info import get_advanced_passing_stats, get_advanced_receiving_stats
...
def verify_migration():
    ...
    print("Verifying get_advanced_passing_stats migration...\n")

    # Test 1: Check function signature matches pattern
    print("Test 1: Function signature")
    passing_sig = inspect.signature(get_advanced_passing_stats)
    receiving_sig = inspect.signature(get_advanced_receiving_stats)

    passing_params = list(passing_sig.parameters.keys())
    receiving_params = list(receiving_sig.parameters.keys())

    assert passing_params == receiving_params, f"Parameter mismatch: {passing_params} vs {receiving_params}"
    print(f"   ✓ Function signature matches pattern: {passing_params}")

    # Test 2: Check function source uses build_player_stats_query
    print("\nTest 2: Implementation uses build_player_stats_query")
    source = inspect.getsource(get_advanced_passing_stats)
    ...
    docstring = get_advanced_passing_stats.__doc__
    assert docstring is not None, "Docstring should be preserved"
    assert "passing stats" in docstring.lower(), "Docstring should mention passing stats"
    assert "QB" in docstring, "Docstring should mention QB default"
    ...
    print("  - Old query logic removed")
    print("  - Correct parameters for passing stats (vw_advanced_passing_analytics, QB position)")
