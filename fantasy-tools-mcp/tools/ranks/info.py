...
def get_fantasy_rank_page_types(supabase: Client) -> list[str]:
    ...
    try:
        ...
        raise Exception(f"Error fetching page_type values: {str(e)}")

# Tool 2: Get dynasty ranks, filtered by position (optional), limited to 150 rows

def get_fantasy_ranks(supabase: Client, position: str = None, page_type: str = None, limit: int = 30) -> list[dict]:
    """
    ...
    """
    columns = [
        "player", "team", "pos", "ecr", "age", "years_of_experience", "team_nfl", "team_full", "player_owned_avg"
    ]
    try:
        ...
        raise Exception(f"Error fetching dynasty ranks: {str(e)}")
