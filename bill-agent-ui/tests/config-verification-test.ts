...
import { retryWithBackoff, RetryExhaustedError } from '../src/lib/ai/toolRetry'
import {
  CircuitBreaker,
  CircuitState,
  CircuitBreakerOpenError
} from '../src/lib/ai/circuitBreaker'
...
async function testRetryMaxAttempts() {
  ...
  try {
    ...
    if (error instanceof RetryExhaustedError) {
      console.log(
        `  ✓ RetryExhaustedError thrown after ${error.attempts} attempts`
      )

      if (error.attempts === 2) {
        console.log(
          '  ✓ PASS: RETRY_MAX_ATTEMPTS=2 correctly limited retries to 2 attempts'
        )
        return true
      } else {
...
async function testRetryInitialDelay() {
  ...
  try {
    ...
    if (callTimes.length >= 2) {
      ...
      console.log(
        `  First retry delay: ${actualDelay}ms (expected: ${expectedDelay}ms)`
      )

      const timing = verifyTiming(actualDelay, expectedDelay)
      console.log(
        `  Timing accuracy: ${timing.accuracy}% (diff: ${timing.diff}ms)`
      )

      if (timing.pass) {
        console.log(
          '  ✓ PASS: RETRY_INITIAL_DELAY_MS=500 correctly set initial delay to 500ms'
        )
        return true
      } else {
...
async function testRetryBackoffMultiplier() {
  ...
  try {
    ...
    if (callTimes.length >= 3) {
      ...
      console.log(
        `  Delay 1 accuracy: ${timing1.accuracy}% (diff: ${timing1.diff}ms)`
      )
      console.log(
        `  Delay 2 accuracy: ${timing2.accuracy}% (diff: ${timing2.diff}ms)`
      )

      if (timing1.pass && timing2.pass) {
        console.log(
          '  ✓ PASS: RETRY_BACKOFF_MULTIPLIER=3 correctly applied 3x exponential backoff'
        )
        return true
      } else {
...
async function testCircuitBreakerThreshold() {
  ...
  if (finalState === CircuitState.OPEN) {
    console.log(
      '  ✓ PASS: CIRCUIT_BREAKER_THRESHOLD=3 correctly opened circuit after 3 failures'
    )
    return true
  } else {
...
async function testCircuitBreakerTimeout() {
  ...
  if (state === CircuitState.HALF_OPEN) {
    console.log(
      '  ✓ PASS: CIRCUIT_BREAKER_TIMEOUT_MS=2000 correctly reset circuit after 2 seconds'
    )
    return true
  } else {
    console.log(
      `  ✗ FAIL: Expected circuit to be HALF_OPEN after timeout, got ${state}`
    )
    return false
  }
}
...
async function runAllTests() {
  ...
  results.push({
    ...
    pass: await testRetryMaxAttempts()
  })
  ...
  results.push({
    ...
    pass: await testRetryInitialDelay()
  })
  ...
  results.push({
    ...
    pass: await testRetryBackoffMultiplier()
  })
  ...
  results.push({
    ...
    pass: await testCircuitBreakerThreshold()
  })
  ...
  results.push({
    ...
    pass: await testCircuitBreakerTimeout()
  })
