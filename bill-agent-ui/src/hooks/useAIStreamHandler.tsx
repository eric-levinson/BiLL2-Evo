  ...
  const handleStreamResponse = useCallback(
    async (input: string | FormData) => {
      ...
      try {
        ...
        if (mode === 'team' && teamId) {
          // Use server proxy which will inject authenticated user_id
          playgroundRunUrl = `/api/proxy/playground/teams/${encodeURIComponent(
            teamId
          )}/runs`
        } else if (mode === 'agent' && agentId) {
          playgroundRunUrl = `/api/proxy/playground/agents/${encodeURIComponent(
            agentId
          )}/runs`
        }
