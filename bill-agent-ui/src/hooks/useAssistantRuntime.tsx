...
export function useAssistantRuntime() {
  ...
  // Deduplicate tool calls for the assistant-ui runtime to prevent
  // "Duplicate key toolCallId-xyz in tapResources" crashes during streaming.
  // deduplicateToolCalls is reference-stable: it returns the original array
  // when no duplicates exist, so React skips re-renders on clean chunks.
  const runtimeMessages = useMemo(
    () => deduplicateToolCalls(messages),
    [messages]
  )
  ...
  const runtime = useAISDKRuntime({ ...chatHelpers, messages: runtimeMessages })
