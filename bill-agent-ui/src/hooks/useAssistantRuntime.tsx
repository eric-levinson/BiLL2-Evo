...
export function useAssistantRuntime() {
  ...
  if (!sessionId && !newChatIdRef.current) {
    ...
    console.log('[Session] Pre-generating ID for new chat:', newChatIdRef.current)
  } else if (sessionId) {
  ...
  const { messages, error, setMessages, isLoading } = chatHelpers
  ...
  useEffect(() => {
    ...
    if (!sessionId && messages.length > 0 && newChatIdRef.current) {
      console.log('[Session] Updating URL with pre-generated ID:', newChatIdRef.current)
  ...
  useEffect(() => {
    ...
    if (sessionId === autoGeneratedSessionIdRef.current) {
      console.log('[Session] Skipping load for auto-generated session:', sessionId)
      return
    }
    ...
    const loadSession = async () => {
      ...
      try {
        ...
        if (session && session.messages) {
          // Deduplicate tool calls to prevent rendering errors
          const deduplicatedMessages = deduplicateToolCalls(session.messages)
          ...
          console.log('[Session] Session not found in database, clearing:', sessionId)
          setSessionId(null)
          setMessages([])
        }
      } catch (err) {
