...
export const getPlaygroundTeamSessionsAPI = async (
  ...
  try {
    // Use the server-side proxy so the authenticated Supabase user.id is
    // injected as `user_id` on the request to the Playground service.
    const proxyUrl = `/api/proxy/playground/teams/${encodeURIComponent(
      teamId
    )}/sessions`
