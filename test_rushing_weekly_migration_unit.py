#!/usr/bin/env python3
"""
Unit test to verify get_advanced_rushing_stats_weekly migration structure.
...
def test_rushing_weekly_stats_structure():
    """Test that get_advanced_rushing_stats_weekly uses build_player_stats_query correctly."""
    print("Testing get_advanced_rushing_stats_weekly migration structure...")
    print("=" * 60)
    ...
    try:
        with patch('tools.metrics.info.build_player_stats_query') as mock_build:
            ...
            mock_build.return_value = {"advRushingStats": []}

            # Call the function with all parameters
            result = get_advanced_rushing_stats_weekly(
                supabase=mock_supabase,
                player_names=["McCaffrey"],
                season_list=[2024],
                weekly_list=[1, 2, 3],
            ...
            assert kwargs["table_name"] == "vw_advanced_rushing_analytics_weekly", f"Wrong table: {kwargs.get('table_name')}"
            print("   ✓ Correct table_name: vw_advanced_rushing_analytics_weekly")
            ...
            assert kwargs["default_positions"] == ["RB", "QB"], f"Wrong default_positions: {kwargs.get('default_positions')}"
            print("   ✓ Correct default_positions: ['RB', 'QB']")

            assert kwargs["return_key"] == "advRushingStats", f"Wrong return_key: {kwargs.get('return_key')}"
            print("   ✓ Correct return_key: advRushingStats")

            assert kwargs["player_names"] == ["McCaffrey"], f"Wrong player_names: {kwargs.get('player_names')}"
            ...
            assert kwargs["metrics"] == ["rushing_yards", "rushing_tds", "carries"], f"Wrong metrics: {kwargs.get('metrics')}"
            print("   ✓ Passes through metrics")

            assert kwargs["order_by_metric"] == "rushing_yards", f"Wrong order_by_metric: {kwargs.get('order_by_metric')}"
            print("   ✓ Passes through order_by_metric")

            assert kwargs["limit"] == 10, f"Wrong limit: {kwargs.get('limit')}"
            print("   ✓ Passes through limit")

            assert kwargs["positions"] == ["RB"], f"Wrong positions: {kwargs.get('positions')}"
            print("   ✓ Passes through positions")

            # Verify return value
            assert result == {"advRushingStats": []}, "Should return result from build_player_stats_query"
...
if __name__ == "__main__":
    success = test_rushing_weekly_stats_structure()
